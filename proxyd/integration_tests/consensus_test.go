...
func setupForcedCandidate(t *testing.T) (map[string]nodeContext, *proxyd.BackendGroup, *ProxydHTTPClient, func()) {
	// setup mock servers
	node1 := NewMockBackend(nil)
	node2 := NewMockBackend(nil)
	...
	// setup proxyd with forced candidate config
	config := ReadConfig("consensus_forced_candidate")
	svr, shutdown, err := proxyd.Start(config)
	require.NoError(t, err)
...
func TestConsensusForcedCandidate(t *testing.T) {
	nodes, bg, _, shutdown := setupForcedCandidate(t)
	defer nodes["node1"].mockBackend.Close()
	defer nodes["node2"].mockBackend.Close()
	defer shutdown()
	...
	t.Run("forced candidate is not polled but included in consensus group", func(t *testing.T) {
		// reset state
		for _, node := range nodes {
			node.handler.ResetOverrides()
			node.mockBackend.Reset()
		...
		// only update node1 (the non-forced candidate)
		// node2 is a forced candidate and should not be polled
		bg.Consensus.UpdateBackend(ctx, nodes["node1"].backend)
		bg.Consensus.UpdateBackendGroupConsensus(ctx)

		// node2 (forced candidate) should be in consensus group
		// even though it was never polled
		consensusGroup := bg.Consensus.GetConsensusGroup()
		require.Contains(t, consensusGroup, nodes["node2"].backend,
			"forced candidate should be in consensus group without being polled")
		require.Contains(t, consensusGroup, nodes["node1"].backend)
		require.Equal(t, 2, len(consensusGroup))

		// consensus block numbers should come from node1 only (the polled backend)
		require.Equal(t, "0x101", bg.Consensus.GetLatestBlockNumber().String())
		require.Equal(t, "0xe1", bg.Consensus.GetSafeBlockNumber().String())
		require.Equal(t, "0xc1", bg.Consensus.GetFinalizedBlockNumber().String())

		// node2 (forced candidate) should have received zero requests
		require.Equal(t, 0, len(nodes["node2"].mockBackend.Requests()),
			"forced candidate should not receive any polling requests")

		// node1 should have received polling requests
		require.Greater(t, len(nodes["node1"].mockBackend.Requests()), 0,
			"non-forced candidate should receive polling requests")
	})

	t.Run("forced candidate does not affect consensus block calculation", func(t *testing.T) {
		// reset state
		for _, node := range nodes {
			node.handler.ResetOverrides()
			node.mockBackend.Reset()
		...
		// consensus should reflect node1's state, not be dragged to 0x0
		// by the unpolled forced candidate
		require.NotEqual(t, "0x0", bg.Consensus.GetLatestBlockNumber().String(),
			"forced candidate with zero block state should not drag consensus to 0x0")
		require.Equal(t, "0x101", bg.Consensus.GetLatestBlockNumber().String())
	})
}
